...
!  Reviewer: Rahul Garg                               Date: 25-Mar-14  C
!
! AB 30/5/16 (2016-1) Set DTSOLID using particle response time  
!                               tested
!
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
...
!-----------------------------------------------
      ...
      USE discretelement
      USE physprop, only: MU_g0, RO_g0, D_P0, RO_S0
      use error_manager
      IMPLICIT NONE
!-----------------------------------------------
! Local Variables
!-----------------------------------------------
      double precision :: DES_TimeTmp
      INTEGER :: M

      CALL INIT_ERR_MSG("CFASSIGN_DEM")

! AB Set dtsolid using particle response time.
      DO M = 1, DES_MMAX
         DES_TimeTmp = ABS(RO_S0(M)-RO_G0)*(D_P0(M)**2)/(18.0*MU_g0)*C(7)
	     IF (DES_TimeTmp<DTSOLID .AND. &
              .NOT.(RO_S0(M)>C(8)*0.99 .AND. RO_S0(M)<C(8)*1.01)) &
                     DTSOLID = DES_TimeTmp
      ENDDO
 ...
 1000 format('Setting collision model parameters for DEM.', /, &
      'DTSOLID based on particle time response = ', 2x, E17.10)
