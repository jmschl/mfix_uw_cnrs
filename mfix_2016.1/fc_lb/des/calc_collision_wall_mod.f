...
! or is already marked as a potential exiting particle
         IF(.NOT.IS_NORMAL(LL)) CYCLE
		  ...
		  ! AB skip if fixed particle
		 IF(RO_Sol(LL)>C(8)*0.99 .AND. RO_Sol(LL)<C(8)*1.01) CYCLE
                ...
                IF ( RO_Sol(LL)/=C(4) ) THEN  
                    ...
                    IF( (DistApart<R_LM/4.0) .AND. (DistApart>ZERO) ) THEN
                        ...
                        CALL CFRELVEL_WALL(LL, V_REL_TRANS_NORM,VREL_T,           &
                               NORMAL, DISTMOD)
                        FORCE_LUB = -6.0*PI*MU_GT(PIJK(LL,4)) &
                                 *DES_RADIUS(LL)**2*V_REL_TRANS_NORM/(DistApart+C(6))
                        FC(LL,:) = FC(LL,:) + DIST(:)*FORCE_LUB/DISTMOD
                    ENDIF
                ENDIF
            ENDIF
