...
!
! AB 30/5/16 (2016-1) Skip fixed particles  
!                              C(8) in mfix.dat is density of fixed particles
!                              tested    
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
...
! AB Skip fixed particles         
            IF ( RO_Sol(L)>C(8)*0.99 .AND. RO_Sol(L)<C(8)*1.01 ) DES_ACC_OLD(L,:) = ZERO
            ROT_ACC_OLD(L,:) = TOW(L,:)
         ENDDO
      ENDIF
...
!!$omp section
 ...
 ! AB Skip fixed particles         
         DO L =1, MAX_PIP
             IF (RO_Sol(L)>C(8)*0.99 .AND. RO_Sol(L)<C(8)*1.01) &
...
!!$omp section
 ...
 ! AB Skip fixed particles         
         DO L =1, MAX_PIP
             IF ( RO_Sol(L)>C(8)*0.99 .AND. RO_Sol(L)<C(8)*1.01 ) THEN
                 DES_VEL_NEW(L,1) = ZERO		
                 DES_VEL_OLD(L,1) = ZERO
             ENDIF
         ENDDO
