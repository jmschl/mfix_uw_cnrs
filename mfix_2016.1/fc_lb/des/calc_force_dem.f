...
! AB 30/5/16 (2016-1) Added particle-particle surface tension cohesive short-range forces
!                     uses C(1-4) in mfix.dat
!                     activated when C(1)=1 and USE_COHESION=.TRUE. in mfix.dat
!                     acts only between particles of density C(4) and other particle classes
!                     added to post-processing mag. of cohesive forces 
!                     untested
! AB 30/5/16 (2016-1) Added particle-particle lubrication short-range force
!                     uses C(4-5)
!                     activated when C(5)=1 in mfix.dat
!                     does not affect particles of density C(4)
!                     based on Izard et al (2014) JFM, equ. 2.18
!                     tested
! AB 30/5/16 (2016-1) skip fixed particles of density C(8)
!                              tested                      
!
! Zack McIntire 06/06/16 (2016-1) Added force chain calculations to
!                                 program.
!                                                                      !
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
#include "version.inc"
...
!---------------------------------------------------------------------//
      ...
      USE visc_g, ONLY: MU_GT    ! AB Add melt viscosity
...
! AB local values used for calculating lubrication force
      DOUBLE PRECISION :: FORCE_LUB
...
!!$omp    hamaker_constant,asperities,surface_energy, force_mag_pair,   &
!!$omp    C,RO_Sol, force_lub, mu_gt  ,                                 &
...
!!$omp do
      ...
      DO LL = 1, MAX_PIP
         ...
         DO CC = CC_START, CC_END-1
		  ...
		  ! AB skip if pair of two fixed particles
            IF(RO_Sol(LL)>C(8)*0.99 .AND. RO_Sol(LL)<C(8)*1.01 .AND. &
                RO_Sol(I)>C(8)*0.99 .AND. RO_Sol(I)<C(8)*1.01) CYCLE
...
!AB Compute particle-particle surface tension cohesive short-range forces
            IF(USE_COHESION .AND. (C(1)==1)) THEN
               IF ( (RO_Sol(LL)/=RO_Sol(I)) .AND. &
                       ( (RO_Sol(LL)==C(4)) .OR. (RO_Sol(I)==C(4)) ) ) THEN
                    DistApart = (SQRT(DIST_MAG)-R_LM)
                    IF(DistApart < C(3)) THEN
                        EQ_RADIUS = 2d0 * DES_RADIUS(LL)*DES_RADIUS(I) /        &
                                           (DES_RADIUS(LL)+DES_RADIUS(I))
	               
                        FORCE_COH = 2.0*PI*C(2)*EQ_RADIUS*(1.0-DistApart/C(3))
                        FC_TMP(:) = DIST(:)*FORCE_COH/SQRT(DIST_MAG)
                        TOW_TMP(:,:) = ZERO
...
!AB Compute particle-particle lubrication short-range force
            IF(C(5)==1) THEN
                IF ( (RO_Sol(LL)/=C(4)) .AND. (RO_Sol(I)/=C(4)) ) THEN  
	                  !not for bubbles
                    DistApart = (SQRT(DIST_MAG)-R_LM)
                    EQ_RADIUS = DES_RADIUS(LL)*DES_RADIUS(I) /        &
                                       (DES_RADIUS(LL)+DES_RADIUS(I))
                    IF( (DistApart<R_LM/4.0) .AND. (DistApart>ZERO) ) THEN
	                      ! Calculate the components of translational relative velocity 
                        CALL CFRELVEL(LL,I,V_REL_TRANS_NORM,VREL_T,NORMAL(:),DIST_MAG)
                        FORCE_LUB = -3.0*PI*(MU_GT(PIJK(LL,4))+MU_GT(PIJK(I,4))) &
                                 *EQ_RADIUS**2*V_REL_TRANS_NORM/(DistApart+C(6))
                        FC_TMP(:) = FC_TMP(:) + DIST(:)*FORCE_LUB/SQRT(DIST_MAG)
                       TOW_TMP(:,:) = ZERO
                    ENDIF
                ENDIF
            ENDIF
...
! just for post-processing mag. of cohesive forces on each particle   AB Added surface tension
      IF(USE_COHESION .AND. (VAN_DER_WAALS .OR. C(1)==1) .AND. GRAV_MAG > ZERO) THEN
