...
!-----------------------------------------------
      ...
      integer :: l, i, k, des_unit, l_pair_num 
      ...
      integer :: wdimn, cc, cc_start, cc_end
...
!   allocate a large array for particle pairs
    allocate(pairs(3,max_pip*30)) ! 1: l, 2: i, 3: cc

!   loop over all particles for contact pairs with non-zero normal contact force
!   need modification when cohesive model is used!!!
    do l = 1, max_pip
       IF(IS_NONEXISTENT(l)) CYCLE

       cc_start = 1
       IF (l.gt.1) CC_START = NEIGHBOR_INDEX(l-1)
       CC_END   = NEIGHBOR_INDEX(l)
       
       do cc = cc_start, cc_end -1
          i = neighbors(cc)
          IF(IS_NONEXISTENT(I)) CYCLE  

!          if((use_cohesion.and.force_mag_pair(3,cc).gt.zero) .or. &
!	        (.not.use_cohesion.and.force_mag_pair(1,cc).gt.zero))then	  

           if(force_mag_pair(1,cc).gt.zero)then
            l_pair_num = l_pair_num + 1
            pairs(1,l_pair_num) = l
            pairs(2,l_pair_num) = i
            pairs(3,l_pair_num) = cc
!            write(*,*)l, i, cc, l_pair_num, max_pip
          endif	   
       enddo
    enddo
	
        allocate( l_fc_p1(l_pair_num,3))
        allocate( l_fc_p2(l_pair_num,3))
	...
	do l=1, l_pair_num
	  l_fc_p1(l,:) = des_pos_new(pairs(1,l),:)
	  l_fc_p2(l,:) = des_pos_new(pairs(2,l),:)
      l_force_n(l) = force_mag_pair(1,pairs(3,l))
      l_force_t(l) = force_mag_pair(2,pairs(3,l))
      l_force_c(l) = force_mag_pair(3,pairs(3,l))          
	enddo    

!   local variables for writing data	
	lglocnt = 10
...
! gather information from all processor
         lcount = 1
         do k = 1,wDIMN     
            dprocbuf(1:llocalcnt) = l_fc_p1(1:llocalcnt,k)
            call desmpi_gatherv(ptype=2)
            ltemp_array(:,lcount) = drootbuf(:); lcount=lcount+1
         end do
         
         do k = 1,wDIMN
            dprocbuf(1:llocalcnt) = l_fc_p2(1:llocalcnt,k)
            call desmpi_gatherv(ptype=2)
            ltemp_array(:,lcount) = drootbuf(:); lcount=lcount+1
         end do
	  ...
	  open(des_unit,file=fname_vtp,status='unknown') 
        ...
        deallocate (l_fc_p1,l_fc_p2,l_force_n,l_force_t,l_force_c,dprocbuf,drootbuf,iprocbuf,irootbuf,ltemp_array,pairs)        
