...
!
! AB 30/5/16 (2016-1) C(1) is a new cohesion model
!                              tested
!                                                                      !
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
...
! Verify that at a cohesion model is specified. AB C(1) is a new cohesion model
      ELSEIF(.NOT.SQUARE_WELL .AND. .NOT.VAN_DER_WAALS .AND. C(1)==0) THEN
         WRITE(ERR_MSG,1003)
         CALL FLUSH_ERR_MSG(ABORT=.TRUE.)
      ENDIF
...
! Modules
!---------------------------------------------------------------------//
      use constant, only: PI, C   ! AB Added C
...
! Calculate the collision time scale.
            TCOLL_TMP = PI/SQRT(KN/MASS_EFF - &
               ((DES_ETAN(M,L)/MASS_EFF)**2)/4.d0)
            		  ! AB skip if fixed particle pair
            IF (.NOT.(RO_S0(L)>C(8)*0.99 .AND. RO_S0(L)<C(8)*1.01 .AND. &
                 RO_S0(M)>C(8)*0.99 .AND. RO_S0(M)<C(8)*1.01)) &
