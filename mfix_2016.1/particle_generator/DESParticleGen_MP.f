...
!
	...
	integer, parameter :: maxp = 2000000 ! Static allocation

	integer dim
	integer random		! 1=random distribution; 0=alligned
        integer accept, touch, ia
 	integer i, j, k, l, iseed
        integer n(100), np(100), np1, nptotal, nt, CurrentNumber
	integer nx, ny, nz
        integer ntypes
	...
	real*8 u, v, w
        ...
        open(unit=10, file="Pgen_MP.in", status='old')
	...
	dia = 2.1*radius	! so that particles don't 
                                ! touch each other to begin with
        rad1 = 1.05*rad
...
!       Specifying the initial distribution of the particles
		 ...
		 go to 101
	      ...
	      x(nt) = 0.5*maxdia + (i-1)*maxdia
              if(xp.gt.xl.or.yp.gt.yl.or.zp.gt.zl) then
                 write(*,*) 'Particle location outside the domain', 
     &            xp, yp, zp
                 stop
              end if
           end do
        else if(random.eq.1) then 
           write(*,*) 'Random option not debugged/correct for',
     &      'multiparticle'
           iseed = 98765432
 ...
 10		 continue
                 CurrentNumber = 0
                 do l = 1, ntypes
                   CurrentNumber = CurrentNumber + np(l)
...
!       setting the velocities to zero 
	...
	w = 0d0
        ...
        print *, "radius is ", ntype_index(1)

        if(dim.eq.2) then
           do i = 1, nptotal
	      write(20,11) x(i), y(i), radius(ntype_index(i)),
     &         density(ntype_index(i)), u, v
           end do
	else
           do i = 1, nptotal
	      write(20,12) x(i), y(i), z(i), radius(ntype_index(i)),
     &         density(ntype_index(i)), u, v, w
           end do
	end if

 11     FORMAT (6(d15.4,2x))
 12     FORMAT (8(d15.4,2x))

        stop
        end program DES_Particle_Generator
